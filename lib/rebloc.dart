...
/// creates output solely through streams. They are given two chances to act in
/// response to incoming actions: first as middleware, and again as a reducer.
///
...
/// subscribe and manipulate the stream to perform middleware and reducer
...
/// next [Bloc] will have a chance to act on it.
///
...
/// emitted on the [Store]'s `states` stream, where changes can be picked up by
/// [ViewModelSubscriber] widgets.
///
/// [ViewModelSubscriber] is similar to [StreamBuilder] in that it listens to a
/// stream and builds widgets in response, but with a few key differences:
///
/// - It looks for a [StoreProvider] ancestor in order to get a reference to a
///   [Store] of matching type.
/// - It assumes the stream will always emit a value immediately on
///   subscription (an RxDart BehaviorSubject is used by [Store] to ensure
///   this). As a result, it has no mechanism for connection states or snapshots
///   that don't contain data.
/// - It converts the app state objects it receives into view models using a
///   required `convert` function, and ignores any changes to the app state that
///   don't cause a change in its view model, limiting rebuilds.
...
///   can be dispatched in response to user actions.
...
///
/// A [Bloc] that does so is expected use the
/// [Action] and [state] provided in any [Accumulator] it receives to calculate
/// a new [state], then emit it in a new Accumulator with the original action
/// and new [state].
class Accumulator<S> {
  final Action action;
  final S state;
...
///
/// A [Store] performs a few main jobs:
///
/// - Create the dispatch/reduce stream using an [initialState] value.
